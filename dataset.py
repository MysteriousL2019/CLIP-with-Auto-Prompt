...
class CLIPDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if (idx < self.initlen):
            ...
            rand_idx = random.randint(0, self.initlen - 1)
            item = {
                key: torch.tensor(values[idx])
                for key, values in self.encoded_captions.items()
            }
            ...
            image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
            image = self.transforms(image=image)['image']
            image_a = cv2.imread(f"{CFG.image_path}/{self.image_filenames[rand_idx]}")
            image_a = cv2.cvtColor(image_a, cv2.COLOR_BGR2RGB)
            image_a = self.transforms(image=image_a)['image']
            avg_image = (image_a + image) / 2

            item['image'] = torch.tensor(avg_image).permute(2, 0, 1).float()
            item['caption'] = self.captions[idx] + " " + self.captions[rand_idx]
            return item

    def __len__(self):
        return len(self.captions)*2
